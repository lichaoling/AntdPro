...
(function(global, factory) {
  ...
  function arcgisToGeoJSON$1(arcgis, idAttribute) {
    ...
    if (arcgis.geometry || arcgis.attributes) {
      ...
      if (arcgis.attributes) {
        geojson.id =
          arcgis.attributes[idAttribute] || arcgis.attributes.OBJECTID || arcgis.attributes.FID;
      }
    }
  ...
  function _getAttributionData(url, map) {
    jsonp(
      url,
      {},
      L$1.Util.bind(function(error, attributions) {
        if (error) {
          return;
        }
  ...
  var BasemapLayer = L$1.TileLayer.extend({
    ...
    getAttribution: function() {
      if (this.options.attribution) {
        var attribution =
          '<span class="esri-dynamic-attribution">' + this.options.attribution + '</span>';
      }
      return attribution;
    },
  });
  ...
  var ImageMapLayer = RasterLayer.extend({
    ...
    _requestExport: function(params, bounds) {
      if (this.options.f === 'json') {
        ...
        params.f = 'image';
        this._renderImage(
          this.options.url + 'exportImage' + L$1.Util.getParamString(params),
          bounds
        );
      }
    },
  ...
  var FeatureManager = VirtualGrid.extend({
    ...
    _filterExistingFeatures: function(oldFrom, oldTo, newFrom, newTo) {
      var layersToRemove =
        oldFrom && oldTo ? this._getFeaturesInTimeRange(oldFrom, oldTo) : this._currentSnapshot;
    ...
    _getMetadata: function(callback) {
      if (this._metadata) {
        ...
        this.metadata(
          L$1.Util.bind(function(error, response) {
    ...
    addFeature: function(feature, callback, context) {
      this._getMetadata(
        L$1.Util.bind(function(error, metadata) {
          if (error) {
            if (callback) {
      ...
      if (this._map) {
        ...
        for (var i in this._layers) {
          if (
            ...
            this._currentSnapshot.indexOf(this._layers[i].feature.id) !== -1
          ) {
      ...
      if (layer && layer.setIcon && this.options.pointToLayer) {
        ...
        if (this.options.pointToLayer) {
          var getIcon = this.options.pointToLayer(
            geojson,
            L$1.latLng(geojson.geometry.coordinates[1], geojson.geometry.coordinates[0])
          );
